...
import sys
sys.path.append('/Pulse_optimization/periodic_sources')
import oscillator_bursting as oscillator_cpp
import random

class PeriodicEnv1D(gym.Env):
    def __init__(self, cfg):
        super(PeriodicEnv1D, self).__init__()
        self.cfg = cfg
#         system parameters
        ...
        self.high_a = np.array([0.97 * self.T])  #6% 18% 6%
        self.low_s = 0.
        self.high_s = 3.14*2
...
#         print(action)
        width_p  = round(float(self.denormalize_actions(action)[0]) / self.dt)
        kfactor = 0#self.denormalize_actions(action)[2]
        gap =  0#round(self.denormalize_actions(action)[1]/ self.dt)
...
#         print('after clear end', oscillator_cpp.Return_end_idx(self.history, self.hist_length)  )
        oscillator_cpp.Make_step3(self.y, self.history, self.phase_oscillator, 1500*7, self.hist_length)  
