...
def Calc_state(phi, idx):
    return _oscillator_cpp.Calc_state(phi, idx)
Calc_state = _oscillator_cpp.Calc_state

def Zm(phi):
    return _oscillator_cpp.Zm(phi)
Zm = _oscillator_cpp.Zm

def Zp(phi):
    return _oscillator_cpp.Zp(phi)
Zp = _oscillator_cpp.Zp

def MeanField(phi, y):
    return _oscillator_cpp.MeanField(phi, y)
MeanField = _oscillator_cpp.MeanField

def Calc_R(y):
    return _oscillator_cpp.Calc_R(y)
Calc_R = _oscillator_cpp.Calc_R

def Calc_Theta(y):
    return _oscillator_cpp.Calc_Theta(y)
Calc_Theta = _oscillator_cpp.Calc_Theta

def bfpulse_EulerStep(phi, y, np):
    return _oscillator_cpp.bfpulse_EulerStep(phi, y, np)
bfpulse_EulerStep = _oscillator_cpp.bfpulse_EulerStep

def copy_vecs(phi, phi_copy):
    return _oscillator_cpp.copy_vecs(phi, phi_copy)
copy_vecs = _oscillator_cpp.copy_vecs

def free_vector(l, nl, nh):
    return _oscillator_cpp.free_vector(l, nl, nh)
free_vector = _oscillator_cpp.free_vector

def Make_step(phi, y, width_p_, gap_, width_n_, pos_Aampl, neg_Aampl, skip_steps):
    return _oscillator_cpp.Make_step(phi, y, width_p_, gap_, width_n_, pos_Aampl, neg_Aampl, skip_steps)
Make_step = _oscillator_cpp.Make_step

def init(nptrans):
    return _oscillator_cpp.init(nptrans)
init = _oscillator_cpp.init

def init_MF():
    return _oscillator_cpp.init_MF()
init_MF = _oscillator_cpp.init_MF

def init_state_copy(phi):
    return _oscillator_cpp.init_state_copy(phi)
init_state_copy = _oscillator_cpp.init_state_copy
